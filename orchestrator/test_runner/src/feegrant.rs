...
use gravity_proto::cosmos_sdk_proto::cosmos::feegrant::v1beta1::{
    BasicAllowance, MsgGrantAllowance, QueryAllowanceRequest,
};
...
pub async fn feegrant_test(gravity_contact: &Contact, keys: Vec<ValidatorKeys>) {
    let granter = get_user_key(None);
    let grantee = get_user_key(None);

    info!(
        "Granter address: {}, grantee address: {}",
        granter.cosmos_address, grantee.cosmos_address
    );

    // Send the granter some coins to use
    let send_res = gravity_contact
        .send_coins(
            DSCoin {
    ...
    assert!(
        send_res.is_ok(),
        "Failed to send coins to granter: {:?}",
        send_res.err()
    );
    ...
    assert!(
        send_res.is_ok(),
        "Failed to send coins to grantee: {:?}",
        send_res.err()
    );
    ...
    assert!(
        res.is_ok(),
        "Failed to grant fee allowance: {:?}",
        res.err()
    );

    let mut fgqc = FeeGrantQueryClient::connect(gravity_contact.get_url())
        .await
        .unwrap();
    let allowance = fgqc
        .allowance(QueryAllowanceRequest {
            granter: granter.cosmos_address.to_string(),
            grantee: grantee.cosmos_address.to_string(),
        })
        .await
        ...
        .into_inner();
    assert!(
        allowance.allowance.is_some(),
        "Allowance should exist after granting"
    );

    let amount = 10u32;
    let fee_amount = 1_000000u64;
    let grantee_balance = gravity_contact
        .get_balance(grantee.cosmos_address, "ugraviton".to_string())
        .await
        .expect("Failed to get grantee balance")
        .unwrap()
        .amount;
    let granter_balance = gravity_contact
        .get_balance(granter.cosmos_address, "ugraviton".to_string())
        .await
        .expect("Failed to get granter balance")
        .unwrap()
        .amount;
    ...
    let args = gravity_contact
        .get_message_args(grantee.cosmos_address, fee, None)
        .await
        .expect("Failed to get message args");
    let result = gravity_contact
    ...
    assert!(
        result.is_ok(),
        "Failed to send message with feegrant: {:?}",
        result.err()
    );
    ...
    let granter_balance = gravity_contact
        ...
        .amount;
    assert_eq!(
        grantee_balance, expect_grantee_balance,
        "Grantee balance incorrect"
    );
    assert_eq!(
        granter_balance, expect_granter_balance,
        "Granter balance incorrect"
    );
