...
use crate::utils::{
    create_gravity_params_proposal, footoken_metadata, get_erc20_balance_safe,
    set_cosmos_bridgeable_tokens, vote_yes_on_proposals, GravityProposalParams, ValidatorKeys,
};
...
pub async fn valset_rewards_test(
    ...
    let reward_formatted = format!("{}{}", valset_reward.amount, valset_reward.denom);
    create_gravity_params_proposal(
        ...
        GravityProposalParams {
            bridge_ethereum_address: Some(gravity_address.to_string()),
            bridge_chain_id: Some("1".to_string()),
            valset_reward_amount: Some(reward_formatted),
            ..Default::default()
        },
    )
    .await;
