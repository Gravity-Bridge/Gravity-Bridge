...
pub async fn get_last_checked_block(
    ...
    while current_block.clone() > 0u8.into() {
            ...
            || logic_call_executed_events.is_err()
        {
            ...
            metrics_error_counter(1, "Failed to get blockchain events while resyncing, is your Eth node working?");
            continue;
        }
        ...
        for event in batch_events {
            match TransactionBatchExecutedEvent::from_log(&event) {
                ...
                Err(e) => {
                    ...
                    metrics_error_counter(1, "Got batch event that we can't parse");
                }
            }
        }
        for event in send_to_cosmos_events {
            match SendToCosmosEvent::from_log(&event) {
                ...
                Err(e) => {
                    ...
                    metrics_error_counter(3, "Got SendToCosmos event that we can't parse");
                }
            }
        }
        for event in erc20_deployed_events {
            match Erc20DeployedEvent::from_log(&event) {
                ...
                Err(e) => {
                    ...
                    metrics_error_counter(3, "Got ERC20Deployed event that we can't parse");
                }
            }
        }
        ...
        for event in valset_events {
            match ValsetUpdatedEvent::from_log(&event) {
                ...
                Err(e) => {
                    ...
                    metrics_error_counter(3, "Got valset event that we can't parse");
                }
            }
        }
