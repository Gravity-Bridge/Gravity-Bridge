...
func (k Keeper) BuildOutgoingTXBatch(
	...
	selectedTx, err := k.pickUnbatchedTX(ctx, evmChainPrefix, contract, maxElements)
	if err != nil {
		return nil, err
...
func (k Keeper) OutgoingTxBatchExecuted(ctx sdk.Context, evmChainPrefix string, tokenContract types.EthAddress, nonce uint64) {
	b := k.GetOutgoingTXBatch(ctx, evmChainPrefix, tokenContract, nonce)
	if b == nil {
	...
	k.IterateOutgoingTXBatches(ctx, evmChainPrefix, func(key []byte, iter_batch types.InternalOutgoingTxBatch) bool {
		// If the iterated batches nonce is lower than the one that was just executed, cancel it
		if iter_batch.BatchNonce < b.BatchNonce && iter_batch.TokenContract.GetAddress() == tokenContract.GetAddress() {
			err := k.CancelOutgoingTXBatch(ctx, evmChainPrefix, tokenContract, iter_batch.BatchNonce)
			if err != nil {
