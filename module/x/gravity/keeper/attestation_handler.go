...
func (a AttestationHandler) handleSendToCosmos(ctx sdk.Context, claim types.MsgSendToCosmosClaim) error {
	...
	if err != nil {
		...
		a.keeper.logger(ctx).Error("Invalid SendToCosmos receiver",
			"address", sdk.AccAddress(receiverAddress).String(),
			"cause", err.Error(),
	...
	isCosmosOriginated, denom := a.keeper.ERC20ToDenomLookup(ctx, claim.EvmChainPrefix, *tokenAddress)
...
func (a AttestationHandler) handleErc20Deployed(ctx sdk.Context, claim types.MsgERC20DeployedClaim) error {
	...
	existingERC20, exists := a.keeper.GetCosmosOriginatedERC20(ctx, claim.EvmChainPrefix, claim.CosmosDenom)
	if exists {
	...
	a.keeper.setCosmosOriginatedDenomToERC20(ctx, claim.EvmChainPrefix, claim.CosmosDenom, *tokenAddress)
...
func (a AttestationHandler) handleValsetUpdated(ctx sdk.Context, claim types.MsgValsetUpdatedClaim) error {
	...
	if claim.ValsetNonce != 0 { // Handle regular valsets
		trustedValset := a.keeper.GetValset(ctx, claim.EvmChainPrefix, claim.ValsetNonce)
		...
		a.keeper.SetLastObservedValset(ctx, claim.EvmChainPrefix, claimSet)
	}
...
func (a AttestationHandler) addToIbcAutoForwardQueue(
	...
	if strings.TrimSpace(cosmosReceiver) == "" {
		panic(fmt.Sprintf("invalid call to addToIbcAutoForwardQueue: invalid or inaccurate cosmosReceiver %s!", cosmosReceiver))
	}
