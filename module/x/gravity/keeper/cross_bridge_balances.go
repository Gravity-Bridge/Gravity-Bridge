...
func (k Keeper) FetchBridgedTokenBalances(ctx sdk.Context) types.InternalERC20Tokens {
	...
	unaccountedBalances := k.UnaccountedGravityModuleBalances(ctx)
	// However, we must ignore any ERC20 tokens since the bank supply is the source of truth for those
	unaccountedLessVouchers := k.CoinsToCosmosOriginatedERC20Tokens(ctx, unaccountedBalances)

	balances.Sort()
	unaccountedLessVouchers.Sort()
	fullyAccounted := balances.SubSorted(unaccountedLessVouchers)
	k.logger(ctx).Info("Collected Snapshot Balances", "all-balances", balances.String(), "transient-balances", unaccountedLessVouchers.String(), "firm-balances", fullyAccounted.String())

	return fullyAccounted
}

// CoinsToCosmosOriginatedERC20Tokens converts the input coins into the relevant set of "Unaccounted ERC20 Tokens", which are
// ERC20 representations of Cosmos tokens (e.g. native GRAV or IBC tokens).
// Note: this function will ignore any input tokens which are Ethereum originated, omitting them from the output
// and similarly any cosmos asset which has no ERC20 representation will not be in the output
func (k Keeper) CoinsToCosmosOriginatedERC20Tokens(ctx sdk.Context, coins sdk.Coins) types.InternalERC20Tokens {
	var tokens types.InternalERC20Tokens

	var token types.InternalERC20Token
	var err error
	for _, c := range coins {
		...
		// Only collect Cosmos Originated tokens
		if isCosmosOriginated {
			token = types.InternalERC20Token{Contract: *erc20, Amount: c.Amount}
			tokens = append(tokens, &token)
		}
	}

	// Since the tokens were appended, consolidate them down to one record per token, only nonzero entries
	tokens.Consolidate()
