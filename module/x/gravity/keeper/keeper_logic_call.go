...
func (k Keeper) GetOutgoingLogicCall(ctx sdk.Context, invalidationID []byte, invalidationNonce uint64) *types.OutgoingLogicCall {
	...
	k.cdc.MustUnmarshal(store.Get(types.GetOutgoingLogicCallKey(EthChainPrefix, invalidationID, invalidationNonce)), &call)
	return &call
}
...
func (k Keeper) SetOutgoingLogicCall(ctx sdk.Context, call types.OutgoingLogicCall) {
	...
	key := types.GetOutgoingLogicCallKey(EthChainPrefix, call.InvalidationId, call.InvalidationNonce)
	if store.Has(key) {
...
func (k Keeper) DeleteOutgoingLogicCall(ctx sdk.Context, invalidationID []byte, invalidationNonce uint64) {
	ctx.KVStore(k.storeKey).Delete(types.GetOutgoingLogicCallKey(EthChainPrefix, invalidationID, invalidationNonce))
}
...
func (k Keeper) IterateOutgoingLogicCalls(ctx sdk.Context, cb func([]byte, types.OutgoingLogicCall) bool) {
	prefixStore := prefix.NewStore(ctx.KVStore(k.storeKey), types.AppendChainPrefix(types.KeyOutgoingLogicCall, EthChainPrefix))
...
func (k Keeper) SetLogicCallConfirm(ctx sdk.Context, msg *types.MsgConfirmLogicCall) {
	...
	ctx.KVStore(k.storeKey).
		Set(types.GetLogicConfirmKey(EthChainPrefix, bytes, msg.InvalidationNonce, acc), k.cdc.MustMarshal(msg))
}
...
func (k Keeper) GetLogicCallConfirm(ctx sdk.Context, invalidationId []byte, invalidationNonce uint64, val sdk.AccAddress) *types.MsgConfirmLogicCall {
	...
	store := ctx.KVStore(k.storeKey)
	data := store.Get(types.GetLogicConfirmKey(EthChainPrefix, invalidationId, invalidationNonce, val))
	if data == nil {
		return nil
	}
...
func (k Keeper) DeleteLogicCallConfirm(
	...
	val sdk.AccAddress) {
	ctx.KVStore(k.storeKey).Delete(types.GetLogicConfirmKey(EthChainPrefix, invalidationID, invalidationNonce, val))
}
...
func (k Keeper) IterateLogicConfirmByInvalidationIDAndNonce(
	...
	store := ctx.KVStore(k.storeKey)
	prefix := types.GetLogicConfirmNonceInvalidationIdPrefix(EthChainPrefix, invalidationID, invalidationNonce)
