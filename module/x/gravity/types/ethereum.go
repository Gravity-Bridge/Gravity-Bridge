...
const (
	...
	// Gravity2DenomPrefix is the denom prefix for Ethereum-originated tokens that were remapped.
	Gravity2DenomPrefix = "gravity2"

	// Gravity2DenomLen is the expected total length of a gravity2 denom string (8 + 42 = 50).
	Gravity2DenomLen = len(Gravity2DenomPrefix) + ETHContractAddressLen
...
func GravityDenomToERC20(denom string) (*EthAddress, error) {
	// Explicitly reject gravity2 denoms before the general HasPrefix check.
	// "gravity2" starts with "gravity" (GravityDenomPrefix), so without this guard a
	// gravity2 denom would reach the address-validation step and fail there for the wrong
	// reason (invalid address) rather than the right one (wrong prefix).
	if strings.HasPrefix(denom, Gravity2DenomPrefix) {
		return nil, fmt.Errorf("denom (%s) uses the gravity2 prefix for remapped tokens, not the gravity prefix", denom)
	}
...
// Gravity2Denom converts an EthAddress to the gravity2-prefixed cosmos denom used for
// Ethereum-originated tokens that were remapped
func Gravity2Denom(tokenContract EthAddress) string {
	return fmt.Sprintf("%s%s", Gravity2DenomPrefix, tokenContract.GetAddress().Hex())
}

// Gravity2DenomToERC20 parses a gravity2-prefixed cosmos denom back to an EthAddress.
// Returns an error if the string does not start with Gravity2DenomPrefix or has the wrong length.
func Gravity2DenomToERC20(denom string) (*EthAddress, error) {
	if !strings.HasPrefix(denom, Gravity2DenomPrefix) {
		return nil, fmt.Errorf("denom (%s) does not have gravity2 prefix", denom)
	}
	if len(denom) != Gravity2DenomLen {
		return nil, fmt.Errorf("len(denom) (%d) not equal to Gravity2DenomLen (%d)", len(denom), Gravity2DenomLen)
	}
	contract := strings.TrimPrefix(denom, Gravity2DenomPrefix)
	ethAddr, err := NewEthAddress(contract)
	if err != nil {
		return nil, fmt.Errorf("error (%s) validating ethereum contract address in gravity2 denom", err)
	}
	return ethAddr, nil
}
...
func (m ERC20ToDenom) ValidateBasic() error {
	...
	if err = ValidateStrictDenom(m.Denom); err != nil {
		return errorsmod.Wrapf(ErrInvalid, "invalid erc20todenom: denom is invalid: %v", err)
	}
