...
func TestMsgSendToCosmosClaim(t *testing.T) {
	...
	for _, v := range keeper.OrchAddrs {
		...
		ethClaim := types.MsgSendToCosmosClaim{
			...
			TokenContract:  tokenETHAddr,
			EthereumSender: anyETHAddr,
			CosmosReceiver: ibcChannel + "/" + myOraiAddr,
			Orchestrator:   v.String(),
			EvmChainPrefix: evmChain.EvmChainPrefix,
		}
...
func TestMsgSendToCosmosClaimSpreadVotes(t *testing.T) {
	...
	for i := range []int{0, 1, 2} {
		...
		assert.NotEqual(t, sdk.Coins{sdk.NewInt64Coin(evmChain.EvmChainPrefix+"0x0bc529c00C6401aEF6D220BE8C6Ea1667F6Ad93e", 12)}, balance1)
	}
	...
	assert.Equal(t, sdk.Coins{sdk.NewInt64Coin(evmChain.EvmChainPrefix+"0x0bc529c00C6401aEF6D220BE8C6Ea1667F6Ad93e", 12)}, balance2)
	...
	assert.Equal(t, sdk.Coins{sdk.NewInt64Coin(evmChain.EvmChainPrefix+"0x0bc529c00C6401aEF6D220BE8C6Ea1667F6Ad93e", 12)}, balance3)
}
