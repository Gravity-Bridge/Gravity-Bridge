...
func (app Gravity) ValidateMembers() {
	...
	if app.icaAuthKeeper == nil {
		panic("Nil icaAuthKeeper!")
	}
	if app.icaControllerKeeper == nil {
		panic("Nil icaControllerKeeper!")
	}
...
func NewGravityApp(
	...
	keys := sdk.NewKVStoreKeys(
		...
		icacontrollertypes.StoreKey, icahosttypes.StoreKey, icaauthtypes.StoreKey,
	)
	...
	scopedICAControllerKeeper := capabilityKeeper.ScopeToModule(icacontrollertypes.SubModuleName)
	...
	// Here the IBC controller module gets a reference to the icaauth module to finalze message routing, this enables
	// the controller to adhere to IBC's single-dispatch routing while also enforcing proper ordering of message handlers
	// See the controller module's ibc_module.go file for additional clarity on IBC Msg handling
	...
	// According to the ICA integration guide, these routes are appropriate. Particularly the ICA controller module
	// must be able to handle the icaauth module's IBC messages before passing control on to the icaauth module
...
func (app *Gravity) registerStoreLoaders() {
	...
	// Register the new ICA module and the special StoreLoader to add them
